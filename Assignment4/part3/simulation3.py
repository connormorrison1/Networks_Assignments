...
if __name__ == '__main__':
    ...
    cost_D = {'H1': {0: 1}, 'RB': {0: 1}} # {neighbor: {interface: cost}}
    router_a = network.Router(name='RA',
                              cost_D = cost_D,
                              max_queue_size=router_queue_size)
    object_L.append(router_a)

    cost_D = {'RD': {0: 1}} # {neighbor: {interface: cost}}
    router_b = network.Router(name='RB',
                              cost_D = cost_D,
                              max_queue_size=router_queue_size)
    object_L.append(router_b)

    cost_D = {'RA': {2: 1}}  # {neighbor: {interface: cost}}
    router_c = network.Router(name='RC',
                              cost_D=cost_D,
                              max_queue_size=router_queue_size)
    object_L.append(router_c)

    cost_D = {'H2': {0: 1}, 'RC': {1: 1}}  # {neighbor: {interface: cost}}
